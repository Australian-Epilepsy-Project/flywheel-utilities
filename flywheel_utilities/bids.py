...
def add_dataset_description(bids_dir):
    ...
    if not description.is_file():
        ...
        log.info("Dummy dataset_description.json created "
                 "in root bids directory")
...
def create_bids_dir(context, subject, modalities, add_description=True):
    ''' Create BIDS directory, and a root level dummy dataset
        description if requested. The BIDS directory will maintain any session
        structure found on Flywheel.
    ...
    Args:
        ...
        modalities (list(str)): list of modalities that need to be downloaded
        add_description (bool): add dummy dataset description
    Return:
        bids_dir (pathlib.Path): path to bids directory
    '''
    ...
    sub_path = context.work_dir / "bids" / ("sub-" + subject.label)
...
def create_deriv_dir(context, sub_label, which_version="first"):
    '''
    Create output folder to store results. The folder name and version are
    retrieved from the manifest label and version. The folder structure follows
    the specification for BIDS derivatives (<pipeline>-v<version>).
    E.g., /fMRIPrep-v21.0.0/sub-XXXXX/

    Args:
        context (flywheel_gear_toolkit.GearToolkitContext): gear context object
        sub_label (str): subject label (XXXXXX)
        which_version (str): if using X.X.X_Y.Y.Y versioning, which position
        is the version of the underlying BIDS App. Specify single if
        only one version present, and specify none if no version is
        to be include

    Returns
        deriv_dir (pathlib.Path): path to derivatives folder
    '''

    gear_name = utils.get_gear_name(context).replace(':', '-v')
    ...
    if which_version != "none":
        ...
        try:
            ...
            log.error("Could not isolate Flywheel versioning in gear name when"
                      " trying to strip it for BIDs derivative directory")
            log.error(f"Retrieved gear name: {gear_name}")
            sys.exit(1)
    else:
        gear_name = gear_name[:gear_name.find('-v')]

    deriv_dir = context.work_dir / gear_name / ('sub-'+sub_label)
